...
2. 选择或创建本地公司数据目录；
3. 配置至少一个模型提供方；
4. 创建 CEO、CTO、Product Lead 最小董事会；
5. 导入一个 Git 仓库；
6. 选择批准预设，默认“平衡”；
7. 创建本地公司，看到一条可执行的起始建议。

数据目录必须在 Control Plane 初始化前由启动它的 host 固定。浏览器通过显式配对连接到 host 已选的数据目录，只读取该服务返回的目录事实，不能在线迁移或更改目录。

首次引导不要求用户预建部门、批量招聘 Agent 或配置复杂工作流。M1 只显示起始建议和董事会能力边界；可发送的董事会会话属于 M2，不能把静态消息或输入框伪装成已实现功能。
...
| ID | 需求 |
|---|---|
| LCP-01 | Control Plane 在回环地址提供认证 API 与事件流 |
...
| LCP-09 | 浏览器连接必须使用本地凭据，不能因 loopback 默认无认证 |
...
桌面生成短期本地凭据并安全传给 WebUI。浏览器连接需要显式认证。服务默认绑定 loopback；监听局域网必须是用户主动配置的高级选项并重新评估威胁模型。
