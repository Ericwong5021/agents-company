...
用户启动本地服务、打开 WebUI 并给出目标后，可以离开页面，让本地公司自行完成：
...
- 共享 WebUI 浏览器工作台；
- 本地常驻 Control Plane、WebUI 状态提示和重启恢复；
...
3. 用户重新打开 WebUI 后读取真实状态；
4. 需要决策、阻塞、完成或异常时在 WebUI 展示高信号状态；
5. WebUI 重新打开或系统重启后恢复项目、Thread、审批、Agent Run 和受管资源；
...
| LCP-03 | WebUI 是唯一产品访问面并使用统一服务契约 |
| LCP-04 | 关闭浏览器页面不停止已授权任务 |
| LCP-05 | WebUI 提供打开工作台、暂停公司和停止任务的控制入口 |
| LCP-06 | WebUI 展示审批、阻塞、完成和异常状态 |
...
员工卡片、组织视图和未来办公室画面都从此投影读取，不各自推断 Agent 状态。
...
1. 启动本地服务并打开 Agent Company WebUI；
2. 配置模型并创建最小董事会；
...
8. 关闭页面后任务继续，重新打开 WebUI 能恢复并定位项目；
