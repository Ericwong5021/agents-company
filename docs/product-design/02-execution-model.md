...
Project = 围绕目标、Charter、团队、资源和验收形成的治理容器
```
...
| Project | 状态、团队、受管资源、策略覆盖、预算 |
| Work Item | 负责人、依赖、输入、输出、能力包、验证器、验收、重试预算 |
...
- 每个写入任务绑定明确资源、作用域、负责人和处置方式；
- 多个 Agent 可读取同一项目，但同一文件、应用对象或工作区的写入必须有所有权；
- 软件项目启用 Worktree 时，每个软件写入任务绑定明确 Worktree；关闭 Worktree 时只允许一个并发写入者；
...
每次失败 Attempt 必须独立保留尝试序号、输入和资源快照、错误、影响、已尝试修复、重试判断、下一次调整和升级记录。后续成功不能覆盖失败事实。

## 8. 软件交付的 Worktree 生命周期
...
- 交付结果已通过对应领域的交付后复核；
- 临时资源已按流程处置；
- 若包含软件交付，合并结果已在主分支验证且 Worktree 已按流程处置；
- 正式决定和风险已回写项目群；
