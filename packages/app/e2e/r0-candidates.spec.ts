...
test("renders the eight R0 human-review screenshot candidates", async ({ page, request }) => {
  ...
  await expect(page.getByLabel("高信号工作流").locator('[data-status="delivered"]')).toHaveText("待验收")
  ...
  await expect(page.locator("#delivery-package").locator("p").filter({ hasText: "1 项成果 · 已核对" })).toBeVisible()
