...
import { createFounderOSGovernanceClient } from "@agents-company/sdk/v2/founder-os"
import { defineAgentCompanyHandler } from "../utils/authenticated-handler"
import { controlPlaneURL } from "../utils/control-plane-client"

export default defineAgentCompanyHandler(async (event) => {
  const companyId = getQuery(event).companyId
  if (typeof companyId !== "string") throw createError({ statusCode: 400, statusMessage: "Company ID is required" })
