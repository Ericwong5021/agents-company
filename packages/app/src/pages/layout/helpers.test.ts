...
import { type Session } from "@agents-company/sdk/v2/client"
import {
  childSessionOnPath,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("agentcompany://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })
  ...
  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("agentcompany://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("agentcompany://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("agentcompany://open-project")).toBeUndefined()
    expect(parseDeepLink("agentcompany://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "agentcompany://open-project?directory=/a",
      "agentcompany://other?directory=/b",
      "agentcompany://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("agentcompany://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("agentcompany://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("agentcompany://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("agentcompany://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "agentcompany://new-session?directory=/a",
      "agentcompany://open-project?directory=/b",
      "agentcompany://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __AGENTCOMPANY__: {
        deepLinks: ["agentcompany://open-project?directory=/a"],
      },
    } as unknown as Window & { __AGENTCOMPANY__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["agentcompany://open-project?directory=/a"])
