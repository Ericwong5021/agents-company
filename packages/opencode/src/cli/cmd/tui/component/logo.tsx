...
const TRACE_IN = 200
const GLOW_OUT = 1600
const SWEEP_INTERVAL = 15000
const SWEEP_DURATION = 2800
const SWEEP_BAND = 4.5
const SWEEP_AMP = 1.5
const PEAK = RGBA.fromInts(245, 251, 255)
...
function wave(x: number, y: number, frame: Frame, live: boolean, ctx: LogoContext) {
  return frame.list.reduce((sum, item) => {
    ...
    const r = ctx.SPAN * (1 - (1 - p) ** EXPAND)
    const fade = (1 - p) ** 1.32
    const j = 1.02 + noise(x + item.x * 0.7, y + item.y * 0.7, item.at * 0.002 + age * 0.12) * 0.52
...
export function Logo(props: { shape?: LogoShape; ink?: RGBA; idle?: boolean; sweep?: boolean } = {}) {
  ...
  const MIMO_BLUE = RGBA.fromInts(60, 147, 255)
          ...
          return (
            <box flexDirection="row" gap={1}>
              <box flexDirection="row">
                {renderLine(line, index(), props.ink ?? MIMO_BLUE, true, 0, frame(), dusk(), idleState())}
              </box>
              <box flexDirection="row">
                {renderLine(
