...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@agents-company/plugin/tui"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir = path.join(ConfigPaths.isLocalConfigDir(source_dir) ? source_dir : Global.Path.config, "themes")
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        // File-based TUI plugins from local Agent Company config directories.
        const configDirs = [path.join(Global.Path.config, "tui")]
