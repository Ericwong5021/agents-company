...
import { Glob } from "@agents-company/shared/util/glob"
...
import type { TuiThemeCurrent } from "@agents-company/plugin/tui"
...
export type BrandColor = "red" | "yellow" | "orange" | "purple" | "blue" | "green" | "pink" | "white"
...
const BRAND_SWATCHES: Record<BrandColor, BrandSwatch> = {
  red: {
    dark: {
      primary: "#ff5d74",
      secondary: "#ff9aa6",
...
function applyBrandColor(theme: Theme, brand: BrandColor | undefined, mode: "dark" | "light") {
  const swatch = BRAND_SWATCHES[brand ?? "blue"][mode]
  const primary = RGBA.fromHex(swatch.primary)
  const secondary = RGBA.fromHex(swatch.secondary)
  const accent = RGBA.fromHex(swatch.accent)
  const success = RGBA.fromHex(swatch.success)
  const info = RGBA.fromHex(swatch.info)
  const glow = RGBA.fromHex(swatch.glow)
  const halo = RGBA.fromHex(swatch.halo)
  const shadow = RGBA.fromHex(swatch.shadow)
  return {
    ...theme,
    primary,
    secondary,
    ...
    success,
    info,
    backgroundPanel: tint(theme.backgroundPanel, shadow, 0.06),
    backgroundElement: tint(theme.backgroundElement, shadow, 0.06),
    backgroundMenu: tint(theme.backgroundMenu, shadow, 0.06),
    borderSubtle: tint(theme.borderSubtle, glow, 0.04),
    border: tint(theme.border, secondary, 0.06),
    borderActive: tint(theme.borderActive, primary, 0.1),
    markdownLink: primary,
    markdownLinkText: accent,
    markdownImage: primary,
    markdownImageText: accent,
    markdownBlockQuote: tint(theme.markdownBlockQuote, halo, mode === "dark" ? 0.05 : 0.02),
    markdownEmph: tint(theme.markdownEmph, accent, 0.04),
    markdownStrong: tint(theme.markdownStrong, glow, 0.03),
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: { mode: "dark" | "light"; plain?: boolean }) => {
    ...
    const values = createMemo<Theme>(() => {
      const brandColor = config.brand_color as BrandColor | undefined
      if (props.plain) {
      ...
      if (active) return applyBrandTheme(resolveTheme(active, store.mode), brandColor, store.mode)
...
async function getCustomThemes() {
  ...
  for (const dir of [Global.Path.config]) {
...
function generateSystem(colors: TerminalColors, mode: "dark" | "light"): ThemeJson {
  ...
  const xiaomiBlue = RGBA.fromInts(60, 147, 255)

  return {
    theme: {
      // Primary colors using Xiaomi lake blue
      primary: xiaomiBlue,
...
function applyBrandTheme(theme: Theme, brand: BrandColor | undefined, mode: "dark" | "light"): Theme {
  const swatch = BRAND_SWATCHES[brand ?? "blue"][mode]
  return {
    ...theme,
    primary: RGBA.fromHex(swatch.primary),
    secondary: RGBA.fromHex(swatch.secondary),
    accent: RGBA.fromHex(swatch.accent),
    success: RGBA.fromHex(swatch.success),
    info: RGBA.fromHex(swatch.info),
    backgroundPanel: tint(theme.backgroundPanel, RGBA.fromHex(swatch.shadow), 0.06),
    backgroundElement: tint(theme.backgroundElement, RGBA.fromHex(swatch.shadow), 0.06),
    backgroundMenu: tint(theme.backgroundMenu, RGBA.fromHex(swatch.shadow), 0.06),
    borderSubtle: tint(theme.borderSubtle, RGBA.fromHex(swatch.glow), 0.04),
    border: tint(theme.border, RGBA.fromHex(swatch.secondary), 0.06),
    borderActive: tint(theme.borderActive, RGBA.fromHex(swatch.primary), 0.1),
    markdownLink: RGBA.fromHex(swatch.primary),
    markdownLinkText: RGBA.fromHex(swatch.accent),
    markdownImage: RGBA.fromHex(swatch.primary),
    markdownImageText: RGBA.fromHex(swatch.accent),
    markdownBlockQuote: tint(theme.markdownBlockQuote, RGBA.fromHex(swatch.halo), mode === "dark" ? 0.05 : 0.02),
    markdownEmph: tint(theme.markdownEmph, RGBA.fromHex(swatch.accent), 0.04),
    markdownStrong: tint(theme.markdownStrong, RGBA.fromHex(swatch.glow), 0.03),
    markdownListItem: RGBA.fromHex(swatch.primary),
    markdownListEnumeration: RGBA.fromHex(swatch.accent),
    markdownCode: RGBA.fromHex(swatch.success),
    syntaxKeyword: RGBA.fromHex(swatch.accent),
    syntaxFunction: RGBA.fromHex(swatch.primary),
    syntaxString: RGBA.fromHex(swatch.success),
    syntaxNumber: RGBA.fromHex(swatch.secondary),
    syntaxType: RGBA.fromHex(swatch.info),
    syntaxOperator: RGBA.fromHex(swatch.info),
  }
}
