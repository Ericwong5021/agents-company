...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.AGENTCOMPANY_TEST ?? "")'), {
      env: {
        AGENTCOMPANY_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "AGENTCOMPANY_TEST_SHELL"], {
      shell: true,
      env: {
        AGENTCOMPANY_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("AGENTCOMPANY_TEST_SHELL=ok")
  })
