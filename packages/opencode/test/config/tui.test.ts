...
test("project config takes precedence over AGENTCOMPANY_TUI_CONFIG (matches AGENTCOMPANY_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.AGENTCOMPANY_TUI_CONFIG = custom
    },
  })
...
test("AGENTCOMPANY_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from AGENTCOMPANY_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.AGENTCOMPANY_CONFIG = path.join(customDir, "mimocode.json")
    },
  })
  const config = await getTuiConfig(tmp.path)
  expect(config.theme).toBeUndefined()
