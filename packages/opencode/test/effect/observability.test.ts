...
const opencodeClient = process.env.AGENTCOMPANY_CLIENT

afterEach(() => {
  if (otelResourceAttributes === undefined) delete process.env.OTEL_RESOURCE_ATTRIBUTES
  else process.env.OTEL_RESOURCE_ATTRIBUTES = otelResourceAttributes

  if (opencodeClient === undefined) delete process.env.AGENTCOMPANY_CLIENT
  else process.env.AGENTCOMPANY_CLIENT = opencodeClient
})
