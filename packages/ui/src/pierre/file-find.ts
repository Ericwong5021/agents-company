...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("control-plane-find")
  api.delete("control-plane-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("control-plane-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("control-plane-find", new Highlight(...rest))
    return true
  }
