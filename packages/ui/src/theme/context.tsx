...
import agentCompanyThemeJson from "./themes/agent-company.json"
...
function resolveThemeId(id: string | null | undefined) {
  if (!id || !knownThemes().has(id)) return
  return id
}
...
const agentCompanyTheme = agentCompanyThemeJson as DesktopTheme
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: { defaultTheme?: string; onThemeApplied?: (theme: DesktopTheme, mode: "light" | "dark") => void }) => {
    const themeId = resolveThemeId(read(STORAGE_KEYS.THEME_ID) ?? props.defaultTheme) ?? "agent-company"
    const colorScheme = (read(STORAGE_KEYS.COLOR_SCHEME) as ColorScheme | null) ?? "system"
    const mode = colorScheme === "system" ? getSystemMode() : colorScheme
    const [store, setStore] = createStore({
      themes: {
        "agent-company": agentCompanyTheme,
    ...
    const onStorage = (e: StorageEvent) => {
      if (e.key === STORAGE_KEYS.THEME_ID && e.newValue) {
        ...
        if (!next) return
        if (next !== "agent-company" && !knownThemes().has(next) && !store.themes[next]) return
        setStore("themeId", next)
        if (next === "agent-company") {
          clear()
          return
        }
    ...
    onMount(() => {
      ...
      const savedTheme = resolveThemeId(rawTheme ?? props.defaultTheme) ?? "agent-company"
    ...
    const setTheme = (id: string) => {
      ...
      if (next !== "agent-company" && !knownThemes().has(next) && !store.themes[next]) {
        console.warn(`Theme "${id}" not found`)
        return
      }
      setStore("themeId", next)
