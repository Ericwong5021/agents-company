CREATE TABLE `company_project_action_v2` (
  ...
  `claimed_at` integer,
  `finished_at` integer,
  CONSTRAINT `company_project_action_action_check` CHECK (`action` IN ('pause_work', 'resume_work', 'stop_work', 'retry', 'resolve_blocker', 'adjust_brief', 'accept_delivery', 'request_change')),
...
INSERT INTO `company_project_action_v2` (
  `id`,
  `project_id`,
  `attention_id`,
  `action`,
...
--> statement-breakpoint
ALTER TABLE `company_project_action_v2` RENAME TO `company_project_action`;
--> statement-breakpoint
