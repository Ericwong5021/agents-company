...
export class Service extends Context.Service<Service, Interface>()("@control-plane/ToolRegistry") {}
      ...
      let filtered = (yield* all()).filter((tool) => {
        if (tool.id === CodeSearchTool.id || tool.id === WebSearchTool.id) {
          if (tool.id === WebSearchTool.id) {
            return (
              input.providerID === ProviderID.controlPlane ||
              input.providerID === "xiaomi" ||
              Flag.AGENTCOMPANY_ENABLE_EXA
            )
          }
          return input.providerID === ProviderID.controlPlane || Flag.AGENTCOMPANY_ENABLE_EXA
        }
