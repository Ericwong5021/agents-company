...
const MANAGED_PLIST_DOMAIN = "ai.agent-company.managed"

// Keys injected by macOS/MDM into the managed plist that are not Control Plane config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/agent-company"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "agent-company")
    default:
      return "/etc/agent-company"
  }
}

export function managedConfigDir() {
  return process.env.AGENTCOMPANY_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
