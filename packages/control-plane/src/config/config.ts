...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in agentcompany config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
const InfoSchema = Schema.Struct({
  ...
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://control-plane.ai/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://control-plane.ai/docs/agents" }),
...
export class Service extends Context.Service<Service, Interface>()("@control-plane/Config") {}

function globalConfigFile() {
  const candidates = ["agent-company.jsonc", "agent-company.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://control-plane.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      let result: Info = pipe(
        ...
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "agent-company.json"))),
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "agent-company.jsonc"))),
      )
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://control-plane.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "AGENTCOMPANY_CONFIG_CONTENT") return "local"
        ...
        const mergeClaudeMcp = Effect.fnUntraced(function* (source: string) {
          ...
          for (const [name, server] of Object.entries(data.mcpServers)) {
            ...
            if (existing && result.mcp_origins?.[name]?.type !== "claude") {
              log.info(`skipped Claude Code MCP server "${name}"; native control-plane MCP with same name already exists.`)
              continue
            }
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            log.debug("fetching remote config", { url: `${url}/.well-known/control-plane` })
            const response = yield* Effect.promise(() =>
              fetch(`${url}/.well-known/control-plane`, { signal: AbortSignal.timeout(1000) }),
            )
            if (!response.ok) {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://control-plane.ai/config.json"
            const source = `${url}/.well-known/control-plane`
        ...
        for (const dir of directories) {
          ...
          // Auto-discovered plugins under shared Agent Company config are already local files, so ConfigPlugin.load
        ...
        if (process.env.AGENTCOMPANY_CONFIG_CONTENT) {
          ...
          log.debug("loaded custom config from AGENTCOMPANY_CONFIG_CONTENT")
        }
        ...
        if (managed) {
          ...
          result = mergeConfigConcatArrays(result, next)
          mergeMcpOrigins(managed.source, next, "control-plane")
        }
        ...
        if (Flag.AGENTCOMPANY_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.AGENTCOMPANY_PERMISSION))
        }
