...
import type { AssistantMessage, Event, ControlPlaneClient, SessionMessageResponse, ToolPart } from "@agents-company/sdk/v2"
...
export async function init({ sdk: _sdk }: { sdk: ControlPlaneClient }) {
  return {
...
export class Agent implements ACPAgent {
  ...
  private sdk: ControlPlaneClient
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `control-plane auth login` in the terminal",
      name: "Login with control-plane",
      id: "control-plane-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // Control Plane stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const controlPlaneProvider = providers.find((p) => p.id === "control-plane")
  if (controlPlaneProvider) {
    if (controlPlaneProvider.models["big-pickle"]) {
      return { providerID: ProviderID.controlPlane, modelID: ModelID.make("big-pickle") }
    }
    const [best] = Provider.sort(Object.values(controlPlaneProvider.models))
    if (best) {
      return {
