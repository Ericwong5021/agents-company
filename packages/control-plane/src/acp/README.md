...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for control-plane.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal control-plane sessions
...
control-plane acp --cwd /path/to/project
```
...
```bash
CONTROL_PLANE_ENABLE_QUESTION_TOOL=1 control-plane acp
```
...
- **Terminal Integration**: Full terminal support via control-plane's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | control-plane acp
```
...
ACP sessions map cleanly to control-plane's internal session model:
