...
async function setupProjectIdEnvironment(workingDir: string): Promise<void> {
  ...
  const localFile = nodePath.join(workingDir, ".agent-company-project-id")
  const idFile = nodePath.join(mainGit, "agent-company-project-id")
  ...
  // Belt-and-suspenders: ensure .git/info/exclude lists the project id marker.
  ...
  const markers = [".agent-company-project-id"].filter((marker) => !existing.includes(marker))
  if (markers.length > 0) {
...
export class Service extends Context.Service<Service, Interface>()("@control-plane/Project") {}
