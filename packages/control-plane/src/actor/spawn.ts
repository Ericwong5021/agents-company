 ...
 * TODO: lift to agentcompany.json config (e.g. actor.maxPreReact) and add per-hook
...
export class Service extends Context.Service<Service, Interface>()("@control-plane/Actor") {}
  ...
  Effect.gen(function* () {
    ...
    const runAgentLoop = Effect.fn("Actor.runAgentLoop")(function* (input: {
      ...
      if (info?.role === "assistant" && info.error) {
        ...
        throw new Error(
          "data" in error && error.data && "message" in error.data
    ...
    const forkWork = (input: {
      ...
      Effect.gen(function* () {
        ...
        // ignored: not reachable in normal usage (agents run sets no such rule, spawn doesn't
