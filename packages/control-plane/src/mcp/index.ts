...
export class Service extends Context.Service<Service, Interface>()("@control-plane/MCP") {}
  ...
  Effect.gen(function* () {
    ...
    const connectTransport = (transport: Transport, timeout: number) =>
      Effect.acquireUseRelease(
        ...
        (t) =>
          Effect.tryPromise({
            try: () => {
              const client = new Client({ name: "agentcompany", version: InstallationVersion })
            ...
            if (isAuthError) {
              ...
              if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
                ...
                return bus
                  .publish(TuiEvent.ToastShow, {
                    title: "MCP Authentication Required",
                    message: `Server "${key}" requires authentication. Run: control-plane mcp auth ${key}`,
                    variant: "warning",
                    duration: 8000,
                  })
                  .pipe(Effect.ignore, Effect.as(undefined))
    ...
    const connectLocal = Effect.fn("MCP.connectLocal")(function* (
      ...
      const transport = new StdioClientTransport({
        ...
        env: {
          ...process.env,
          ...(cmd === "control-plane" ? { BUN_BE_BUN: "1" } : {}),
          ...mcp.environment,
        },
      })
