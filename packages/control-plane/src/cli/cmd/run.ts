...
import { createControlPlaneClient, type ControlPlaneClient, type ToolPart } from "@agents-company/sdk/v2"
...
export const RunCommand = cmd({
  ...
  builder: (yargs: Argv) => {
    return yargs
      ...
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to AGENTCOMPANY_SERVER_PASSWORD)",
      })
      .option("dir", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    async function session(sdk: ControlPlaneClient) {
    ...
    async function share(sdk: ControlPlaneClient, sessionID: string) {
      const cfg = await sdk.config.get()
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.AGENTCOMPANY_AUTO_SHARE && !args.share) return
    ...
    if (args.attach) {
      ...
      const sdk = createControlPlaneClient({ baseUrl: args.attach, directory, headers })
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createControlPlaneClient({ baseUrl: "http://control-plane.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
