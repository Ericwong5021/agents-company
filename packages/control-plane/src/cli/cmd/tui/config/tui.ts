...
export class Service extends Context.Service<Service, Interface>()("@control-plane/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  // Every config dir we may read from: global config dir, local Agent Company
  // folders between cwd and home, and AGENTCOMPANY_CONFIG_DIR.
...
async function load(text: string, configFilepath: string): Promise<Info> {
  return ConfigVariable.substitute({ text, type: "path", path: configFilepath, missing: "empty" })
    ...
    .then((data) => {
      ...
      // (mirroring the server config shape) still get their settings applied.
