...
const TUI_SCHEMA_URL = "https://control-plane.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from agent-company.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const configs = await agentCompanyFiles(input)
...
async function agentCompanyFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "agent-company"),
    ...(await Filesystem.findUp(["agent-company.json", "agent-company.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "agent-company"))
  }
  if (Flag.AGENTCOMPANY_CONFIG) files.push(Flag.AGENTCOMPANY_CONFIG)
