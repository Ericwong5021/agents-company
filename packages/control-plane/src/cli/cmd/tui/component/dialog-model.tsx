...
export function useConnected() {
  const sync = useSync()
  return createMemo(() =>
    sync.data.provider.some((x) => x.id !== "control-plane" || Object.values(x.models).some((y) => y.cost?.input !== 0)),
  )
}
...
export function DialogModel(props: { providerID?: string }) {
  ...
  const options = createMemo(() => {
        ...
        return [
          {
            ...
            category,
            disabled: provider.id === "control-plane" && model.id.includes("-nano"),
            footer: model.cost?.input === 0 && provider.id === "control-plane" ? "Free" : undefined,
            onSelect: () => {
    ...
    const providerOptions = pipe(
      sync.data.provider,
      sortBy(
        (provider) => provider.id !== "control-plane",
      ...
      flatMap((provider) => {
        ...
        const models = pipe(
          ...
          map(([model, info]) => ({
            ...
            disabled: provider.id === "control-plane" && model.includes("-nano"),
            footer: info.cost?.input === 0 && provider.id === "control-plane" ? "Free" : undefined,
            onSelect() {
    ...
    const popularProviders: ReturnType<typeof providers> = []
