...
import type { GlobalEvent } from "@agents-company/sdk/v2"
...
declare global {
  const CONTROL_PLANE_WORKER_PATH: string
}
...
async function target() {
  if (typeof CONTROL_PLANE_WORKER_PATH !== "undefined") return CONTROL_PLANE_WORKER_PATH
...
export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start agentcompany tui",
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start agentcompany in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
  ...
  handler: async (args) => {
    ...
    try {
      ...
      const env = sanitizedProcessEnv({
        [AGENTCOMPANY_PROCESS_ROLE]: "worker",
        [AGENTCOMPANY_RUN_ID]: ensureRunID(),
      })
