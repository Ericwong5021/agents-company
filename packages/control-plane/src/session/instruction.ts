...
const FILES = [
  "AGENTS.md",
  ...(Flag.AGENTCOMPANY_DISABLE_CLAUDE_CODE_PROMPT ? [] : ["CLAUDE.md"]),
...
function globalFiles() {
  ...
  if (Flag.AGENTCOMPANY_CONFIG_DIR) {
    files.push(path.join(Flag.AGENTCOMPANY_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.AGENTCOMPANY_DISABLE_CLAUDE_CODE_PROMPT) {
...
export class Service extends Context.Service<Service, Interface>()("@control-plane/Instruction") {}
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const relative = Effect.fnUntraced(function* (instruction: string) {
        ...
        if (!Flag.AGENTCOMPANY_CONFIG_DIR) {
          log.warn(
            `Skipping relative instruction "${instruction}" - no AGENTCOMPANY_CONFIG_DIR set while project config is disabled`,
          )
          return []
        }
        return yield* fs
          .globUp(instruction, Flag.AGENTCOMPANY_CONFIG_DIR, Flag.AGENTCOMPANY_CONFIG_DIR)
          .pipe(Effect.catch(() => Effect.succeed([] as string[])))
      })
