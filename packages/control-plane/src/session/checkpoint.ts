...
export interface Interface {
   ...
   * the CLI shutdown path so headless `agents run` invocations don't exit
...
export class Service extends Context.Service<Service, Interface>()("@control-plane/SessionCheckpoint") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const tryStartCheckpointWriter: (
      ...
      // and same-message.id thinking blocks aren't split. Control Plane's ToolPart
    ...
    const renderRebuildContext = Effect.fn("SessionCheckpoint.renderRebuildContext")(function* (
      ...
      const actors = (
        yield* Effect.forEach(
          [sessionID, ...(yield* session.children(sessionID)).map((child) => child.id)],
          (actorSessionID) => actorRegistry.listBySession(actorSessionID),
          { concurrency: "unbounded" },
        )
      )
        .flat()
        .filter((actor) => actor.background && (actor.status === "pending" || actor.status === "running"))
