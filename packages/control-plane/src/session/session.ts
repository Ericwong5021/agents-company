...
export function plan(input: { slug: string; time: { created: number } }) {
  return path.join(Global.Path.data, "company", "plans", [input.time.created, input.slug].join("-") + ".md")
}
...
export class Service extends Context.Service<Service, Interface>()("@control-plane/Session") {}
...
export const layer: Layer.Layer<Service, never, Bus.Service | Storage.Service | ActorRegistry.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createNext = Effect.fn("Session.createNext")(function* (input: {
      ...
      if (!Flag.AGENTCOMPANY_EXPERIMENTAL_WORKSPACES) {
