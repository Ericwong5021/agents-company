...
export type Source = "cc" | "codex"
export const ALL_SOURCES: readonly Source[] = ["cc", "codex"] as const
...
export async function runAll(opts?: {
  ...
  const totals = {
    scanned: result.cc.scanned + result.codex.scanned,
    imported: result.cc.imported + result.codex.imported,
    resynced: result.cc.resynced + result.codex.resynced,
    errors: result.cc.errors.length + result.codex.errors.length,
  }
