 ...
 * TODO: lift to agentcompany.json config (e.g. session.maxGoalReact).
 */
...
function isExtensionPath(filePath: string): boolean {
  return /\/\.agent-company\/(tools?|skills?|hooks?)\//.test(filePath)
}
...
export class Service extends Context.Service<Service, Interface>()("@control-plane/SessionPrompt") {}
    ...
    const capture: typeof prefixCaptureRef.current = (input) =>
      Effect.gen(function* () {
        ...
        const [skills, env, instructions] = yield* Effect.all([
          sys.skills(ag, captureSession.companyAgentID),
    ...
    const runLoop: (sessionID: SessionID, agentID?: string, task_id?: string) => Effect.Effect<MessageV2.WithParts> = Effect.fn(
      ...
      function* (sessionID: SessionID, agentID?: string, task_id?: string) {
        ...
        while (true) {
          ...
          // (explore/general/etc) spawned via agentcompany's shared-sessionID
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            if (isForkAgent) {
              ...
              // JSDoc in packages/control-plane/src/actor/spawn.ts for the full contract.
              const queryParts =
