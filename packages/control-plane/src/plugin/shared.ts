...
export const DEPRECATED_PLUGIN_PACKAGES = ["control-plane-openai-codex-auth", "control-plane-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, controlPlaneVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(controlPlaneVersion) || semver.major(controlPlaneVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines["control-plane"]
  if (typeof range !== "string") return
  if (!semver.satisfies(controlPlaneVersion, range)) {
    throw new Error(`Plugin requires control-plane ${range} but running ${controlPlaneVersion}`)
  }
}
