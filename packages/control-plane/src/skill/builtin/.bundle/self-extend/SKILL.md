...
You can extend your own capabilities by writing files to `.agentcompany/`. Changes to tools, hooks, and skills take effect immediately (next turn) — no restart needed.

## Creating Tools

Write to `.agentcompany/tools/<name>.ts`:

```ts
import { tool } from "@agents-company/plugin"
...
Write to `.agentcompany/hooks/<name>.ts` — export a Hooks object:
...
```ts
// .agentcompany/tools/bash.ts — overrides built-in bash
import { tool } from "@agents-company/plugin"
...
Write to `.agentcompany/skills/<name>/SKILL.md`:
