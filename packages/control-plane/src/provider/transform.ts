...
export const OUTPUT_TOKEN_MAX = Flag.AGENTCOMPANY_EXPERIMENTAL_OUTPUT_TOKEN_MAX || 32_000
...
// See docs/cache-policy.md and upstream control-plane#26786.
...
function limitImages(msgs: ModelMessage[]): ModelMessage[] {
  const maxImages = Flag.AGENTCOMPANY_MAX_PROMPT_IMAGES
  const maxSize = Flag.AGENTCOMPANY_MAX_PROMPT_IMAGE_SIZE
...
export function options(input: {
  ...
  if (
    ...
    (input.model.providerID === "control-plane" && ["kimi-k2-thinking", "glm-4.6"].includes(input.model.api.id))
  ) {
  ...
  if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
    ...
    if (input.model.providerID.startsWith("control-plane")) {
      result["promptCacheKey"] = input.sessionID
