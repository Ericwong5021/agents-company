...
test.serial("S20 reopens complete old or new graph state at every transaction boundary", async () => {
  await using directory = await tmpdir()
...
test.serial("S27 completes startup reconciliation before dispatch and converges the projection watermark", async () => {
  await using directory = await tmpdir()
