...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under control-plane.", () => {
    expect(paramToAttributeKey("name")).toBe("control-plane.name")
    expect(paramToAttributeKey("slug")).toBe("control-plane.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["control-plane.sessionID"]).toBeUndefined()
    expect(attrs["control-plane.messageID"]).toBeUndefined()
    expect(attrs["control-plane.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under control-plane. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["control-plane.name"]).toBe("exa")
