...
const managedConfigDir = process.env.AGENTCOMPANY_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "agent-company.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "agent-company.json") {
...
test("loads Claude Code MCP servers from home and project config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(config.mcp_origins?.context7?.type).toBe("control-plane")
...
test("ignores legacy tui keys in control-plane config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://control-plane.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "agent-company.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["AGENTCOMPANY_CONSOLE_TOKEN"]
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["AGENTCOMPANY_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["AGENTCOMPANY_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "agent-company.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .agentcompany directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const controlPlaneDir = path.join(dir, ".agentcompany")
      await fs.mkdir(controlPlaneDir, { recursive: true })
      const agentDir = path.join(controlPlaneDir, "agent")
...
test("loads agents from .agentcompany/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const agentsDir = path.join(controlPlaneDir, "agents")
...
test("loads commands from .agentcompany/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const commandDir = path.join(controlPlaneDir, "command")
...
test("loads commands from .agentcompany/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const commandsDir = path.join(controlPlaneDir, "commands")
...
test("agentcompany command overrides .claude command on name collision", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
...
from claude`,
      ...
      await Filesystem.write(
        ...
        `---
description: agentcompany version
---
from agentcompany`,
      )
    },
...
test("does not try to install dependencies in read-only AGENTCOMPANY_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.AGENTCOMPANY_CONFIG_DIR
  process.env.AGENTCOMPANY_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.AGENTCOMPANY_CONFIG_DIR
    else process.env.AGENTCOMPANY_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable AGENTCOMPANY_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "agent-company.json"),
        JSON.stringify({ $schema: "https://control-plane.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const controlPlaneDir = path.join(projectDir, ".agentcompany")
      await fs.mkdir(controlPlaneDir, { recursive: true })
      ...
      await Filesystem.write(
        path.join(controlPlaneDir, "agent-company.json"),
        JSON.stringify({
          $schema: "https://control-plane.ai/config.json",
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/control-plane")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/control-plane")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "agent-company.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-control-plane@2.4.3", file)).toBe("oh-my-control-plane@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-control-plane@2.4.3", "file:///project/.agentcompany/plugin/oh-my-control-plane.js"]
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const controlPlaneDir = path.join(projectDir, ".agentcompany")
        const pluginDir = path.join(controlPlaneDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("AGENTCOMPANY_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["AGENTCOMPANY_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["AGENTCOMPANY_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          const controlPlaneDir = path.join(dir, ".agentcompany", "command")
          await fs.mkdir(controlPlaneDir, { recursive: true })
          await Filesystem.write(path.join(controlPlaneDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          const hasProjectControlPlane = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectControlPlane).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["AGENTCOMPANY_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["AGENTCOMPANY_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["AGENTCOMPANY_CONFIG_DIR"]
      process.env["AGENTCOMPANY_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["AGENTCOMPANY_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["AGENTCOMPANY_CONFIG_DIR"]
      } else {
        process.env["AGENTCOMPANY_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("AGENTCOMPANY_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["AGENTCOMPANY_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["AGENTCOMPANY_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["AGENTCOMPANY_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["AGENTCOMPANY_CONFIG_DIR"] = configDirTmp.path
...
describe("AGENTCOMPANY_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in AGENTCOMPANY_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["AGENTCOMPANY_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["AGENTCOMPANY_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://control-plane.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["AGENTCOMPANY_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["AGENTCOMPANY_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in AGENTCOMPANY_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["AGENTCOMPANY_CONFIG_CONTENT"]
  ...
  const config = ConfigParse.schema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "Control Plane Managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.schema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://control-plane.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://control-plane.ai/config.json")
})
