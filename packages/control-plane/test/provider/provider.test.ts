...
async function provide<R>(input: {
  directory: string
  ...
  ;(Global.Path as { config: string }).config = input.directory
  await AppRuntime.runPromise(Config.Service.use((config) => config.invalidate(true)))
  try {
    return await Instance.provide(input)
  } finally {
...
function controlPlaneProviderPresent(providers: Awaited<ReturnType<typeof list>>): boolean {
  return providers[ProviderID.make("control-plane")] !== undefined
}
        ...
        JSON.stringify({
          ...
          provider: {
            "cloudflare-ai-gateway": {
              options: {
                metadata: { invoked_by: "test", project: "control-plane" },
              },
            },
          },
        }),
...
test("plugin config providers persist after instance dispose", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const root = path.join(dir, ".agentcompany", "plugin")
...
test("control-plane and control-plane-go providers are disabled by MimoFreeAuthPlugin", async () => {
  ...
  const providers = await provide({
    directory: base.path,
    fn: async () => list(),
  })

  // MimoFreeAuthPlugin always pushes control-plane/control-plane-go into disabled_providers,
  ...
  // control-plane/control-plane-go remain loaded. Gate these assertions on mimo provider presence.
  const mimo = providers[ProviderID.make("mimo")]
  if (mimo) {
    expect(controlPlaneProviderPresent(providers)).toBe(false)
    expect(providers[ProviderID.make("control-plane-go")]).toBeUndefined()
