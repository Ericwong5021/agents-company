...
describe("invalid-output continuation — integration", () => {
  test("empty stop step is nudged, second call produces a non-empty final assistant", async () => {
    ...
    try {
      await using _providerSettings = await writeConfig(tmp.path, stub.origin)
        ...
        fn: () =>
          run(
            Effect.gen(function* () {
              ...
              expect(stub.captures.length).toBe(Flag.AGENTCOMPANY_INVALID_OUTPUT_CONTINUATION_LIMIT + 1)
              expect(result.info.role).toBe("assistant")
