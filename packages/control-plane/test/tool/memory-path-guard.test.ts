...
describe("assertMemoryWriteAllowed", () => {
  ...
  test("rejects another project memory path", () => {
    expect(() =>
      assertMemoryWriteAllowed({
        target: path.join(DATA_ROOT, "projects", "p_other", "MEMORY.md"),
        agentName: "build",
        dataRoot: DATA_ROOT,
        projectID: PROJECT_ID,
        sessionID: SESSION_ID,
      }),
    ).toThrow(/outside the current project/)
  })

  test("rejects another session memory path", () => {
    expect(() =>
      assertMemoryWriteAllowed({
        target: path.join(DATA_ROOT, "sessions", "other", "notes.md"),
        agentName: "build",
        dataRoot: DATA_ROOT,
        projectID: PROJECT_ID,
        sessionID: SESSION_ID,
      }),
    ).toThrow(/outside the current session/)
  })
  ...
  describe("checkpoint-writer allowlist (v5)", () => {
    test("memory.md at <pid>/ allowed for checkpoint-writer", () => {
      const target = path.join(DATA_ROOT, "projects", PROJECT_ID, "memory.md")
      expect(() =>
        assertMemoryWriteAllowed({
          target,
    ...
    test("memory-<topic>.md at <pid>/ allowed (spillover)", () => {
      const target = path.join(DATA_ROOT, "projects", PROJECT_ID, "memory-rules.md")
      expect(() =>
        assertMemoryWriteAllowed({
          target,
          agentName: "checkpoint-writer",
    ...
    test("pinned.md at <pid>/ NOW rejected (renamed to memory.md)", () => {
      const target = path.join(DATA_ROOT, "projects", PROJECT_ID, "pinned.md")
      expect(() =>
        assertMemoryWriteAllowed({
          target,
          agentName: "checkpoint-writer",
    ...
    test("non-memory-prefix .md at <pid>/ rejected for checkpoint-writer", () => {
      const target = path.join(DATA_ROOT, "projects", PROJECT_ID, "notes.md")
      expect(() =>
        assertMemoryWriteAllowed({
          target,
          agentName: "checkpoint-writer",
