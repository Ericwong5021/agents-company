...
describe("installation", () => {
  describe("method", () => {
    test("detects npm when @agents-company/control-plane is in npm list output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "npm" && args.includes("-g")) return "@agents-company/control-plane@1.0.0"
          return ""
        },
      )
    ...
    test("detects pnpm when @agents-company/control-plane is in pnpm list output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "pnpm" && args.includes("-g")) return "@agents-company/control-plane@1.0.0"
          return ""
        },
      )
    ...
    test("detects bun when @agents-company/control-plane is in bun pm ls output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "bun" && args.includes("-g")) return "@agents-company/control-plane@1.0.0"
          return ""
        },
      )
    ...
    test("returns unknown when no package manager has @agents-company/control-plane", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        () => "",
      )
  ...
  describe("latest", () => {
    test("reads version from npm registry for npm method", async () => {
      const layer = testLayer(
        (req) => {
          expect(req.url).toContain(encodeURIComponent("@agents-company/control-plane"))
    ...
    test("resolves version from GitHub releases redirect for curl method", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "curl" && args.includes("https://github.com/Ericwong5021/agents-company/releases/latest"))
            return "HTTP/2 302\r\nlocation: https://github.com/Ericwong5021/agents-company/releases/tag/v0.1.1\r\n"
          return ""
        },
      )
  ...
  describe("upgrade", () => {
    test("runs npm install with correct package", async () => {
      ...
      expect(capturedArgs).toContain("@agents-company/control-plane@2.0.0")
    })
