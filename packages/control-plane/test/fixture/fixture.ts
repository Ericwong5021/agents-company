...
export async function providerTmpdir<T>(options?: TmpDirOptions<T>) {
  const directory = await tmpdir(options)
  const settings = await provideProjectProviderSettings(directory.path)
  return {
    path: directory.path,
