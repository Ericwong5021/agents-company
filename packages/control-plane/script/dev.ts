...
// checkout (at ../../mimoapi/packages/control-plane/src/ext), it is copied into
...
const overlaySrc = path.resolve(pkgDir, "../../mimoapi/packages/control-plane/src/ext")
...
const proc = Bun.spawn(["bun", "run", "--conditions=browser", "src/index.ts", ...process.argv.slice(2)], {
  ...
  env: { ...process.env, AGENTCOMPANY_HOME: process.env.AGENTCOMPANY_HOME ?? path.resolve(pkgDir, "../../.dev-home") },
})
